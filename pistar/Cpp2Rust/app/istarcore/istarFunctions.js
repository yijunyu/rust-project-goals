    ...
    function _layout() {
	  ...
	  var added_elements = {}
          _.forEach(istar.graph.getElements(), function (element) {
			...
			added_elements[id] = element
                }
		if (element.isDependum()) {
			var id = element.prop('id')
			var name = element.prop('name')
			var size = element.prop('size')
		        g.setNode(id, {
			...
			if (added_elements[id] == null) {
			      added_elements[id] = element
			}
		}
          });
          _.forEach(istar.graph.getLinks(), function (link) {
			var source = link.prop('source/id')
			var target = link.prop('target/id')
			if (added_elements[source]!=null || added_elements[target]!=null) {
	  ...
	  g.nodes().forEach(function (v) {
		...
		var position = element.prop('position')
			...
			var position = { x: node.x, y: node.y};
            		var cell = istar.graph.getCell(id);
			var orig_position = cell.prop('position')
                    	cell.prop('position', position);
			if (element.isKindOfActor()) {
	                    _.forEach(element.getEmbeddedCells(), function (innerElement) {
				var inner_position = innerElement.prop('position');
				var type = innerElement.prop('type')
				switch (type) {
				    case "Goal":
				    case "Resource":
				    ...
				    case "Task":
					var position = {
					    x: inner_position.x + node.x - orig_position.x,
					    y: inner_position.y + node.y - orig_position.y
					};
					innerElement.prop('position', position);
				    break;
				default:
